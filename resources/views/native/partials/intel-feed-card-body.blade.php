...
<row class="w-full rounded-md overflow-hidden border border-theme-outline-variant bg-theme-surface">
    ...
    <column class="flex-1 p-4 gap-3">
        ...
        <column class="w-full h-px bg-theme-outline-variant"/>
        ...
        <row class="w-full items-center">
            ...
            <pressable ref="helpful-{{ $post->id }}" @press="markHelpful({{ $post->id }})"
                ...
                <row class="items-center gap-2 px-4 py-3 rounded-md border {{ in_array($post->id, $voted, true) ? 'border-theme-primary bg-theme-primary/15' : 'border-theme-outline' }}">
